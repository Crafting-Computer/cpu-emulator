...
var $author$project$CpuEmulator$StepComputer = function (a) {
	return {$: 'StepComputer', a: a};
};
	...
	return $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				model.isRunningComputer ? A2($elm$time$Time$every, 10, $author$project$CpuEmulator$StepComputer) : $elm$core$Platform$Sub$none
			]));
};
...
var $author$project$AsmParser$predefinedSymbolTable = $elm$core$Dict$fromList(
	_List_fromArray(
		[
			...
			_Utils_Tuple2(
			'KBD',
			A2($elm$core$Basics$pow, 2, 20) + A2($elm$core$Basics$pow, 2, 19)),
			_Utils_Tuple2('SP', 0),
			_Utils_Tuple2('LCL', 1),
			_Utils_Tuple2('ARG', 2)
		]));
...
var $author$project$CpuEmulator$stepAInstruction = F2(
	function (number, computer) {
		return _Utils_update(
			...
			{
				aRegister: number,
				mRegister: A2($author$project$CpuEmulator$fetchFromMemory, number, computer.ram),
				pc: computer.pc + 1
			});
	});
var $author$project$CpuEmulator$boolToString = function (b) {
	return b ? '1' : '0';
};
...
var $author$project$CpuEmulator$compute = F2(
	function (computationBits, computer) {
		var m = computer.mRegister;
		var d = computer.dRegister;
		var computationStr = A2(
			$elm$core$String$join,
			'',
			A2($elm$core$List$map, $author$project$CpuEmulator$boolToString, computationBits));
		var a = computer.aRegister;
		switch (computationStr) {
			case '0101010':
				return 0;
			case '0111111':
				return 1;
...
var $author$project$CpuEmulator$getBit = F2(
	function (index, bits) {
		return A2(
			$elm$core$Maybe$withDefault,
			false,
			A2($elm_community$list_extra$List$Extra$getAt, index, bits));
	});
var $author$project$CpuEmulator$moveProgramCounter = F3(
	function (jumpBits, computationResult, computer) {
		...
		var lt = A2($author$project$CpuEmulator$getBit, 0, jumpBits);
		var gt = A2($author$project$CpuEmulator$getBit, 2, jumpBits);
...
var $author$project$CpuEmulator$storeComputationResult = F3(
	function (destinationsBits, result, computer) {
		var storeToMRegister = A2($author$project$CpuEmulator$getBit, 2, destinationsBits);
		var storeToDRegister = A2($author$project$CpuEmulator$getBit, 1, destinationsBits);
		var storeToARegister = A2($author$project$CpuEmulator$getBit, 0, destinationsBits);
		var newMRegister = storeToMRegister ? result : computer.mRegister;
		var newDRegister = storeToDRegister ? result : computer.dRegister;
		var newARegister = storeToARegister ? result : computer.aRegister;
		return _Utils_update(
			computer,
			{
				aRegister: newARegister,
				dRegister: newDRegister,
				mRegister: newMRegister,
				ram: A3($author$project$CpuEmulator$storeToMemory, computer.aRegister, newMRegister, computer.ram)
			});
	});
...
var $author$project$CpuEmulator$stepCInstruction = F2(
	function (instructionBits, computer) {
		var jumpBits = A2(
			$elm$core$List$take,
			3,
			A2($elm$core$List$drop, 10, instructionBits));
		var destinationsBits = A2(
			$elm$core$List$take,
			3,
			A2($elm$core$List$drop, 7, instructionBits));
		var computationBits = A2($elm$core$List$take, 7, instructionBits);
...
var $author$project$CpuEmulator$step = function (computer) {
	var _v0 = A2($elm$core$Array$get, computer.pc, computer.rom);
	if (_v0.$ === 'Just') {
		var instructionStr = _v0.a;
		var _v1 = A2($author$project$Assembler$assembleInstruction, computer.pc, instructionStr);
		if (_v1.$ === 'Err') {
			var err = _v1.a;
			return _Utils_update(
				computer,
				{
					error: $elm$core$Maybe$Just(
						_Utils_Tuple2(computer.pc, err))
				});
		} else {
			var instruction = _v1.a;
			var instructionBinary = $icidasset$elm_binary$Binary$fromIntegers(
				A2(
					$elm$core$List$map,
					A2(
						$elm$core$Basics$composeL,
					...
					A2($elm$core$String$split, '', instruction)));
			var instructionBits = $icidasset$elm_binary$Binary$toBooleans(instructionBinary);
			var computer1 = function () {
				var _v3 = computer.error;
				if (_v3.$ === 'Nothing') {
					return computer;
				} else {
					return _Utils_update(
			...
			if (!instructionBits.b) {
				return computer1;
			} else {
				var opCode = instructionBits.a;
				return (!opCode) ? A2(
					$author$project$CpuEmulator$stepAInstruction,
					$icidasset$elm_binary$Binary$toDecimal(instructionBinary),
					computer1) : A2(
					$author$project$CpuEmulator$stepCInstruction,
					A2($elm$core$List$drop, 19, instructionBits),
					computer1);
			}
		}
	} else {
			...
			_Utils_update(
				model,
				{
					computer: _Utils_update(
						oldComputer,
						{
							rom: A2($elm$core$Array$append, updatedPartOfRom, restOfOldRom)
						}),
					isEditingProgram: false
				}),
					...
					columns: _List_fromArray(
						[
							...
							view: F2(
								function (index, cell) {
									var isEditing = function () {
										var _v3 = model.editingInstructionIndex;
										if (_v3.$ === 'Nothing') {
											return false;
										} else {
									...
									var cellStyle = function (previousStyle) {
										...
										if (_v1.$ === 'Nothing') {
											...
											return _Utils_eq(location, index) ? _Utils_ap(
												...
												_List_fromArray(
													[
														$mdgriffith$elm_ui$Element$Background$color($author$project$CpuEmulator$colors.lightOrange)
													])) : previousStyle;
										}
									}(
										_Utils_eq(index, model.computer.pc) ? _Utils_ap(
											commonStyle,
											_List_fromArray(
												[
													$mdgriffith$elm_ui$Element$Background$color($author$project$CpuEmulator$colors.lightGreen)
									...
									return A2(
										...
										isEditing ? A2(
													...
													])),
											{
												...
												onChange: $author$project$CpuEmulator$EditInstruction(index),
												placeholder: $elm$core$Maybe$Nothing,
												text: cell
											}) : A2(
											$mdgriffith$elm_ui$Element$el,
...
var $author$project$CpuEmulator$StartRunningComputer = {$: 'StartRunningComputer'};
var $author$project$CpuEmulator$StopRunningComputer = {$: 'StopRunningComputer'};
...
var $author$project$CpuEmulator$viewRunButton = function (isRunningComputer) {
	var _v0 = A2($elm$core$Debug$log, 'AL -> isRunningComputer', isRunningComputer);
	return A2(
		$mdgriffith$elm_ui$Element$Input$button,
		$author$project$CpuEmulator$styles.button,
		{
			...
			onPress: isRunningComputer ? $elm$core$Maybe$Just($author$project$CpuEmulator$StopRunningComputer) : $elm$core$Maybe$Just($author$project$CpuEmulator$StartRunningComputer)
		});
};
...
var $author$project$CpuEmulator$viewSingleStepButton = A2(
	...
	$author$project$CpuEmulator$styles.button,
	{
		...
		onPress: $elm$core$Maybe$Just(
			$author$project$CpuEmulator$StepComputer(
				...
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									A2($author$project$CpuEmulator$viewRegister, 'A', model.computer.aRegister),
									A2($author$project$CpuEmulator$viewRegister, 'D', model.computer.dRegister),
									A2($author$project$CpuEmulator$viewRegister, 'M', model.computer.mRegister)
								]))
						])),
					A2(
					$mdgriffith$elm_ui$Element$column,
								...
								[
									...
									_List_fromArray(
										[
											...
											$author$project$CpuEmulator$viewRunButton(model.isRunningComputer),
